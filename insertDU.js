/* hourly_* → daily_* */
const clickhouse = require('./config/clickhouse');
const SEGMENT_LIST = require('./config/segmentList');
const dayjs = require('dayjs');

async function run() {
  const input = process.argv[2];
  let start, end;

  if (input) {
    const isValid = /^\d{4}-\d{2}-\d{2}$/.test(input);
    if (!isValid) {
      console.error("❌ 형식 오류: YYYY-MM-DD 형식이어야 합니다.");
      process.exit(1);
    }
    start = dayjs(input).startOf('day');
    end = start.add(1, 'day');
  } else {
    end = dayjs().startOf('day');
    start = end.subtract(1, 'day');
  }

  const startStr = start.format('YYYY-MM-DD 00:00:00');
  const endStr = end.format('YYYY-MM-DD 00:00:00');
  const dateStr = start.format('YYYY-MM-DD');

  for (const { type } of SEGMENT_LIST) {
    console.log(`\n[일간 집계 시작] ${type}: ${dateStr}`);

    await insertClickSummary(type, startStr, endStr, dateStr);
    await insertTopElements(type, startStr, endStr, dateStr);
    await insertUserDistribution(type, startStr, endStr, dateStr);

    console.log(`[일간 집계 완료] ${type}`);
  }

  await clickhouse.close();
}

// 1. 클릭 요약 통계
async function insertClickSummary(type, start, end, dateStr) {
  const q = `
    INSERT INTO klicklab.daily_click_summary
    SELECT
      toDate('${dateStr}') AS date,
      '${type}' AS segment_type,
      segment_value,
      sum(total_clicks) AS total_clicks,
      sum(total_users) AS total_users,
      round(sum(total_clicks) / sum(total_users), 1) AS avg_clicks_per_user,
      sdk_key
    FROM klicklab.hourly_click_summary
    WHERE segment_type = '${type}'
      AND date_time >= toDateTime('${start}')
      AND date_time < toDateTime('${end}')
    GROUP BY segment_value, sdk_key
  `;
  await clickhouse.command({ query: q });
}

// 2. Top 클릭 요소 (Top 3)
async function insertTopElements(type, start, end, dateStr) {
  const q = `
    INSERT INTO klicklab.daily_top_elements
    SELECT *
    FROM (
      SELECT
        toDate('${dateStr}') AS date,
        '${type}' AS segment_type,
        segment_value,
        element,
        sum(total_clicks) AS total_clicks,
        sum(user_count) AS user_count,
        row_number() OVER (
          PARTITION BY sdk_key, segment_value ORDER BY sum(total_clicks) DESC
        ) AS rank,
        sdk_key
      FROM klicklab.hourly_top_elements
      WHERE segment_type = '${type}'
        AND date_time >= toDateTime('${start}')
        AND date_time < toDateTime('${end}')
      GROUP BY segment_value, element, sdk_key
    )
    WHERE rank <= 3
  `;
  await clickhouse.command({ query: q });
}

// 3. 사용자 분포
async function insertUserDistribution(type, start, end, dateStr) {
  const q = `
    INSERT INTO klicklab.daily_user_distribution
    SELECT
      toDate('${dateStr}') AS date,
      '${type}' AS segment_type,
      segment_value,
      dist_type,
      dist_value,
      sum(user_count) AS user_count,
      sdk_key
    FROM klicklab.hourly_user_distribution
    WHERE segment_type = '${type}'
      AND date_time >= toDateTime('${start}')
      AND date_time < toDateTime('${end}')
    GROUP BY segment_value, dist_type, dist_value, sdk_key
  `;
  await clickhouse.command({ query: q });
}

// 실행
run().catch((err) => {
  console.error('❌ 집계 실패:', err.message);
  process.exit(1);
});
